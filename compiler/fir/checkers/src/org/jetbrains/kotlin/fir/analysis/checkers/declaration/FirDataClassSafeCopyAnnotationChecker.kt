...
object FirDataClassConsistentDataCopyAnnotationChecker : FirClassChecker(MppCheckerKind.Common) {
    override fun check(declaration: FirClass, context: CheckerContext, reporter: DiagnosticReporter) {
        val consistentCopy =
            declaration.annotations.firstOrNull { it.fqName(context.session) == StandardNames.CONSISTENT_DATA_COPY }
        val inconsistentCopy =
            declaration.annotations.firstOrNull { it.fqName(context.session) == StandardNames.INCONSISTENT_DATA_COPY }
            ...
            inconsistentCopy != null && (declaration !is FirRegularClass || !declaration.isData) -> {
                ...
                if (consistentCopy != null &&
                    ...
                    reporter.reportOn(
                        ...
                        StandardNames.CONSISTENT_DATA_COPY,
                        context
                    )
                }

                if (inconsistentCopy != null &&
                    context.languageVersionSettings.supportsFeature(LanguageFeature.IgnoreInconsistentDataCopyAnnotation)
                ) {
                    reporter.reportOn(
                        inconsistentCopy.source,
                        FirErrors.REDUNDANT_ANNOTATION,
                        StandardNames.INCONSISTENT_DATA_COPY,
                        context
                    )
                }
            }
