...
class FirStatusResolver(
    ...
    private fun resolveVisibility(
        ...
        if (containingClass?.status?.isData == true &&
            ...
            return when {
                !session.languageVersionSettings.supportsFeature(LanguageFeature.IgnoreInconsistentDataCopyAnnotation) &&
                        containingClass.annotations.any { it.fqName(session) == StandardNames.INCONSISTENT_DATA_COPY } ->
                    Visibilities.Public
                session.languageVersionSettings.supportsFeature(LanguageFeature.DataClassCopyRespectsConstructorVisibility) ||
                        containingClass.annotations.any { it.fqName(session) == StandardNames.CONSISTENT_DATA_COPY } ->
